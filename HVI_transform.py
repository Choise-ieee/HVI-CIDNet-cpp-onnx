...
class RGB_HVI(nn.Module):
    def __init__(self):
        ...
        # 修复1: 使用标量参数，避免形状问题
        self.density_k = torch.nn.Parameter(torch.tensor(0.2))  # 标量而非[1]
        self.gated = False
        self.gated2 = False
        self.alpha = 1.0
        self.alpha_s = 1.3
    ...
    def HVIT(self, img):
        ...
        # 修复2: 使用torch.zeros_like或torch.empty_like，避免创建新tensor
        B, _, H, W = img.shape
        hue = torch.zeros(B, H, W, device=device, dtype=dtypes)
